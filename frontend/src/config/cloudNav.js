...
function registerMeta(item, section) {
  ROUTE_META_MAP.set(item.to, { section, title: item.label, cloud: Boolean(item.h5Path) });
}

for (const section of CLOUD_NAV_SECTIONS) {
  for (const item of section.items) {
    registerMeta(item, section.label);
  }
}
for (const item of LOCAL_NAV_SECTION.items) {
  ROUTE_META_MAP.set(item.to, { section: LOCAL_NAV_SECTION.label, title: item.label, cloud: false });
}
  ...
  for (const item of allCloudItems) {
    ...
    routes.push({
      ...
      meta: {
        ...
        h5Path: item.h5Path,
        title: item.label,
        section: getRouteMeta(item.to)?.section,
      },
    });
  }
  return routes;
