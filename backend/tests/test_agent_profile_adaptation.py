...
def test_subagent_prompt_follows_profile():
    social = _profile(
        id="douyin-social-roam",
        inherit_workflow_prompt=False,
        system_prompt="只 invoke social-roam。",
    )
    prompt = build_subagent_system_prompt(social)
    assert "social-roam" in prompt
...
def test_experience_filtered_by_agent_profile(experience_store):
    ...
    experience_store.create(
        ...
        AgentExperienceCreate(
            id="exp-social",
            title="漫游",
            task_keywords=["淋浴房", "social-roam"],
            outcome="success",
            lesson="漫游专用",
            platform="douyin",
            agent_profile_id="douyin-social-roam",
        ),
    )
    ...
    daily_items = experience_store.retrieve_for_task(
        ...
        platform="douyin",
        limit=5,
        agent_profile_id="douyin-daily-roam",
    )
