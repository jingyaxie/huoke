...
from app.services.social_roam.types import parse_social_roam_params, render_template
...
def test_parse_social_roam_params_defaults():
    params = parse_social_roam_params({"keyword": "淋浴房"}, platform="douyin")
    assert params.platform == "douyin"
    assert params.keyword == "淋浴房"
    assert params.max_replies == 5
