...
DEFAULT_GLOBAL_RULES: list[dict] = [
    ...
    },
    {
        ...
        "content": (
            ...
            "- 搜索时使用搜索框 `[data-e2e=\"searchbar-input\"]` 而非直接构造未知 API\n"
            ...
            "评论用 browser_get_network_data(url_contains=comment/list)，搜索用 url_contains=search\n"
            "- 全量评论抓取请用内置技能 crawl-video-comments\n"
            "- 搜索视频列表请用 /search-videos 或 skill_search_videos"
        ),
        "always_apply": True,
