...
class DouyinCrawler:
    _interactive_login_task: asyncio.Task | None = None

    def __init__(self, settings: Settings) -> None:
        self.settings = settings

    async def _launch_context(self, headless: bool | None = None) -> tuple[Playwright, Browser, BrowserContext, Page]:
        playwright = await async_playwright().start()
        launch_args = ["--no-sandbox", "--disable-setuid-sandbox", "--disable-dev-shm-usage"]
        browser = await playwright.chromium.launch(
            headless=self.settings.douyin_headless if headless is None else headless,
            args=launch_args,
        )
        kwargs = {"viewport": {"width": 1440, "height": 1200}}
        if self.settings.douyin_storage_state_path.exists():
            kwargs["storage_state"] = str(self.settings.douyin_storage_state_path)
        context = await browser.new_context(**kwargs)
        page = await context.new_page()
        return playwright, browser, context, page

    async def login_and_save_cookies(self, show_browser: bool = True) -> None:
        playwright, browser, context, page = await self._launch_context(headless=not show_browser)
        try:
            ...
            for _ in range(60):
                ...
                if cookie_names & REQUIRED_LOGIN_COOKIES:
                    await context.storage_state(path=str(self.settings.douyin_storage_state_path))
                    return
                await page.wait_for_timeout(3000)
            await context.storage_state(path=str(self.settings.douyin_storage_state_path))
            raise RuntimeError("未检测到有效登录态，请在服务器浏览器里完成扫码/验证后重试。")
        finally:
            await context.close()
            await browser.close()
    ...
    async def start_interactive_login_session(self) -> dict:
        task = DouyinCrawler._interactive_login_task
        if task and not task.done():
            return {"status": "running", "message": "服务器登录窗口已在运行"}
        DouyinCrawler._interactive_login_task = asyncio.create_task(self._run_interactive_login_session())
        return {"status": "started", "message": "服务器登录窗口已启动"}
    ...
    async def fetch_hot(self, limit: int = 100) -> list[CrawlItem]:
        playwright, browser, context, page = await self._launch_context()
        try:
            await page.goto(self.settings.douyin_hot_url, wait_until="domcontentloaded", timeout=120000)
            try:
                await page.wait_for_load_state("networkidle", timeout=10000)
            except Exception:
                # Douyin pages often keep long-lived requests, so networkidle may never arrive.
                pass
    ...
    async def _extract_from_page(self, page: Page, limit: int) -> list[CrawlItem]:
        ...
        for index in range(min(count, 500)):
            ...
            if not text or len(text) < 4:
                continue
            href_text = href or ""
            # Keep only real content links; skip footer/legal links.
            ...
            item = CrawlItem(
                ...
                raw_data={"text": text, "href": href, "index": index},
            )
            results.append(item)
    ...
    def _parse_hot_card_text(self, text: str) -> dict:
        # Typical text shape on /hot cards:
        # "00:24 1.6万 标题内容 @作者 · 1周前"
        ...
        if author_match:
            ...
            # Fallback: remove trailing date-like token and keep the middle as title
