...
- 生产抓取优先 builtin：*-keyword-comments、content-comments、search-content、follow-user、send-dm、reply-comment
...
class AgentService:
    ...
    def _resolve_client(self, provider: str) -> tuple[AsyncOpenAI | None, str | None]:
        ...
        return self.ai_factory.openai(), self.settings.openai_model

    def _resolve_vision_model(self, provider: str, default_model: str) -> str | None:
        if not self.settings.agent_vision_enabled or provider != "openai":
            return None
        return self.settings.agent_vision_model or default_model
    ...
    def _failure_guidance(failure_type: str) -> str:
        mapping = {
            ...
            "generic_error": "连续失败，请切换工具策略并复用现有证据，不要重复同一动作。",
        }
    ...
    def _should_block_redundant_search(
        *,
        ...
        if fn_name in {"skill_search_videos"}:
            return True
        if fn_name == "invoke_skill" and str(fn_args.get("skill_id") or "").strip() == "search-videos":
            return True
        ...
        return {
            ...
            "providers": {
                "openai": {
                    ...
                    "vision": bool(self.settings.openai_api_key and self.settings.agent_vision_enabled),
                ...
                for tool_call in tool_calls:
                    ...
                    if last_tool.get("role") == "tool":
                        try:
                            ...
                            last_result = {}
                        if last_result.get("status") == "failed" and (
                            fn_name.startswith("skill_") or fn_name == "invoke_skill"
                        ):
                            terminal_status = "failed"
                            terminal_summary = last_result.get("error") or "技能执行失败"
                            break
