...
class CommentCrawlerService:
    ...
    async def crawl_video_comments(self, video_url: str, show_browser: bool = False) -> tuple[dict, Path]:
        ...
        payload = await self._fetch_video_comments(video_url=video_url, headless=not show_browser)
        output = self.settings.report_output_dir / f"comments_{aweme_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    async def crawl_keyword_comments(
        ...
        for url in video_urls:
            payload, output = await self.crawl_video_comments(url, show_browser=False)
    ...
    async def search_videos_by_keyword(
        ...
        async with async_playwright() as p:
            ...
            has_storage_state = self.settings.douyin_storage_state_path.exists()
            ...
            async def on_response(resp):
                try:
                    url = resp.url
                    if "/aweme/v1/web/general/search" not in url and "/aweme/v1/web/search/item" not in url:
                        return
