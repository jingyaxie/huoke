...
from app.platforms.xiaohongshu.js_constants import PLATFORM, _build_follow_url, _build_unfollow_url
...
class XhsFollowTool(XhsJsApiTool):
    ...
    async def _relation_on_page(self, page, *, user_id: str, username: str, action: str) -> dict:
        ...
        page_followed = await self._detect_follow_status_on_page(page)
        if page_followed == "followed":
            follow_status_before = "followed"
        ...
        page_after = await self._detect_follow_status_on_page(page)
        if page_after in {"followed", "none"}:
            result["follow_status_after"] = page_after
        return result
    ...
    async def _detect_follow_status_on_page(self, page) -> str:
        try:
    ...
    async def _commit_follow(self, page, *, user_id: str, follow_status_before: str) -> dict:
        ...
        if await self._detect_follow_status_on_page(page) == "followed":
    ...
    async def _commit_unfollow(self, page, *, user_id: str, follow_status_before: str) -> dict:
        if follow_status_before != "followed":
            return {
                "ok": True,
                "skipped": True,
    ...
    async def _follow_via_ui(self, page) -> dict:
        ...
        try:
            ...
            followed = await self._detect_follow_status_on_page(page) == "followed"
        ...
        for label in ("已关注", "互相关注"):
            ...
            if await btn.count():
                try:
                    await btn.click()
                    await page.wait_for_timeout(2000)
                    unfollowed = await self._detect_follow_status_on_page(page) != "followed"
