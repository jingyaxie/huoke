...
class DouyinCommentTool(DouyinJsApiTool):
    ...
    def _use_video_url_ui_passive(
        *,
        ...
        if ui_passive:
            return True
        mode = str((raw_params or {}).get("capture_mode") or "").strip().lower()
        if mode in {"video_url_ui_passive", "ui_passive"}:
            return True
        return not headless
    ...
    async def _fetch_video_comments(
        ...
        *,
        ...
        use_ui = self._use_video_url_ui_passive(
            headless=headless,
            ui_passive=ui_passive,
            raw_params=raw_params,
        )
